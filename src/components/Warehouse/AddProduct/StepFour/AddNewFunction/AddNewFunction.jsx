...
// utils
import {
	searchForFunctionInSupabase,
	insertNewFunctionInSupabase,
} from '../../../../../utils/warehouse'
// redux-thunk
import { fetchFunction } from '../../../../../store/warehouseStore/warehouse-thunk'
// store
...
const AddNewFunctionModal = ({ isOpen, closeModal }) => {
	...
	const onSubmitFormHandler = handleSubmit(data => {
		...
		const insertData = {
			description: data.productFunction,
			state: 1,
		}

		insertNewFunctionInSupabase(insertData).then(response => {
	...
	useEffect(() => {
		...
		const onSearchBrandNameInSupabase = setTimeout(() => {
			searchForFunctionInSupabase(inputQuery).then(response => {
				// hide spinner
				setShowSpinner(false)

				if (response.length > 0) {
					if (
						response[0].description.toLowerCase() === inputQuery.toLowerCase()
					) {
						setError('productFunction', {
							type: 'custom',
							message:
								'Esta función ya se encuentra registrada en la base de datos',
						})
								...
								<div className='mt-2'>
									<form className='w-full' onSubmit={onSubmitFormHandler}>
										<h3 className='text-lg font-medium leading-6 text-gray-900 border-b pb-2 mb-4'>
											Ingresar Nueva Función
										</h3>

										{/* helpful information for the user */}
										<div className='w-full bg-gray-50 border-l-4 border-gray-200 py-2 px-4 mb-4 '>
											<p className='text-gray-500 text-sm font-semibold flex flex-col'>
												<span className='flex'>
													<IconInfoHexagon size={18} className='mr-2' />
													El nombre de la función debe iniciar con letra
													mayúscula.
												</span>
												<span>
													Ejemplo:
										...
										<div className='flex flex-wrap -mx-3 mb-4'>
											<div className='w-full px-3 mb-6 md:mb-0'>
												...
												<p className='text-red-500 text-xs italic'>
													{errors.productFunction &&
														errors.productFunction.message}
												</p>
											</div>
										</div>
...
export default AddNewFunctionModal
